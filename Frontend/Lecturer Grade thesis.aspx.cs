...
{
    public partial class Grade_thesis : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand lecturerGradesStudent = new SqlCommand("LecGradeThesis", conn);
            lecturerGradesStudent.CommandType = CommandType.StoredProcedure;

            SqlParameter StudentID = lecturerGradesStudent.Parameters.Add(new SqlParameter("@sid", SqlDbType.Int));
            StudentID.Value = Int16.Parse(GradedStudentIDIN.Text);
            SqlParameter lecid = lecturerGradesStudent.Parameters.Add(new SqlParameter("@Lecturer_id", SqlDbType.Int));
            lecid.Value = Session["ID"];
            SqlParameter Grade = lecturerGradesStudent.Parameters.Add(new SqlParameter("@Lecturer_grade", SqlDbType.Decimal));
            Grade.Value = decimal.Parse(GradeByLecturerIN.Text);

            conn.Open();
            lecturerGradesStudent.ExecuteNonQuery();
            conn.Close();
        }
    }
}